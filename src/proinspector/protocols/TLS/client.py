...
class TLSClient(_TLSAutomaton):
    ...
    def tls13_Certificate(self,cert, key):
        self.mycert = cert
        self.mykey = key
        if cert:
            self.mycert = cert
    ...
    def tls13_ClientCertificateVerify2(self,cert,key):
        self.mycert = cert
        self.mykey = key
        if cert:
            self.mycert = cert
    ...
    def tls13_UnknownCertificateVerify(self):
        self.add_msg(
            TLSCertificateVerify(sig=_TLSSignature(sig_alg="md5+anon", sig_val=b"Unknown CV"))
        )
